namespace ConsoleWorldRPG.Commands
{
    public static class LootCommands
    {
        ...
        {
            if (input == "look corpses") { ShowCorpses(player); return true; }
            if (input == "loot") { LootFirstCorpse(player); return true; }
            return false;
        }
        ...
        private static void ShowCorpses(Player player)
        {
            ...
            if (!corpses.Any()) { Console.WriteLine("There are no corpses here."); return; }
        ...
        {
            var corpse = player.CurrentRoom.Corpses.FirstOrDefault(c => !c.IsLooted && c.Loot.Any());
            if (corpse == null) { Console.WriteLine("There are no lootable corpses here."); return; }
